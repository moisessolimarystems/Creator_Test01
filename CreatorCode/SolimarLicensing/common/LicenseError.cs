...
namespace Solimar
{
	namespace Licensing
	{
		public class CSolLicHRESULT
		{
			...
			public const uint SP_PROTOCOL_NOT_INSTALLED = 76;
			public const uint SP_NO_LEASE_FEATURE = 101;
			public const uint SP_LEASE_EXPIRED = 102;
			public const uint SP_COUNTER_LIMIT_REACHED = 103;
			public const uint SP_NO_DIGITAL_SIGNATURE = 104;
			public const uint SP_SYS_FILE_CORRUPTED = 105;
			public const uint SP_INVALID_DEV_CODE = 107;
			public const uint SP_DEVID_DOES_NOT_MATCH = 108;
			public const uint SP_DEVICE_SHARING_DETECTED = 109;
			public const uint SP_SERVER_VERSION_NOT_SUPPORTED = 110;
			public const uint SP_FILE_NOT_FOUND = 111;
			public const uint SP_PATH_TOO_LONG = 112;
			public const uint SP_SOFT_DB_CORRUPT = 113;
			public const uint SP_SOFT_DB_RESTORE_DETECTED = 114;
			public const uint SP_PRST_DATA_CORRUPT = 115;
			public const uint SP_SECURITY_RUNTIME_NOT_DETECTED = 116;
			public const uint SP_TIME_TAMPER_DETECTED = 117;
			public const uint SP_END_DATE_REACHED = 118;
			public const uint SP_START_DATE_NOT_REACHED = 119;
			public const uint SP_HOST_ID_ERROR = 120;
			public const uint SP_LIC_INVALID = 121;
			public const uint SP_LIC_RUN_TIME_ERROR = 122;
			public const uint SP_LIC_MEMORY_CORRUPTED = 123;
			public const uint SP_NOT_ENOUGH_MEMORY = 124;
			public const uint SP_IP_ADDRESS_BLOCKED = 125;
			public const uint SP_SERVER_OUT_OF_WORKING_TIME = 126;
			public const uint SP_SECURITY_RUNTIME_VERSION_MISMATCH = 127;
			...
			private const uint EHR_LIC_MAIL_SERVER = ITF_LIC_SVR_OFFSET+52;
			private const uint EHR_DONGLE_EMULATOR = ITF_LIC_SVR_OFFSET+53;
			...
			private const uint EHR_SP_PROTOCOL_NOT_INSTALLED = ITF_LIC_RNBO_OFFSET+SP_PROTOCOL_NOT_INSTALLED;
			private const uint EHR_SP_NO_LEASE_FEATURE = ITF_LIC_RNBO_OFFSET+SP_NO_LEASE_FEATURE;
			private const uint EHR_SP_LEASE_EXPIRED = ITF_LIC_RNBO_OFFSET+SP_LEASE_EXPIRED;
			private const uint EHR_SP_COUNTER_LIMIT_REACHED = ITF_LIC_RNBO_OFFSET+SP_COUNTER_LIMIT_REACHED;
			private const uint EHR_SP_NO_DIGITAL_SIGNATURE = ITF_LIC_RNBO_OFFSET+SP_NO_DIGITAL_SIGNATURE;
			private const uint EHR_SP_SYS_FILE_CORRUPTED = ITF_LIC_RNBO_OFFSET+SP_SYS_FILE_CORRUPTED;
			private const uint EHR_SP_INVALID_DEV_CODE = ITF_LIC_RNBO_OFFSET+SP_INVALID_DEV_CODE;
			private const uint EHR_SP_DEVID_DOES_NOT_MATCH = ITF_LIC_RNBO_OFFSET+SP_DEVID_DOES_NOT_MATCH;
			private const uint EHR_SP_DEVICE_SHARING_DETECTED = ITF_LIC_RNBO_OFFSET+SP_DEVICE_SHARING_DETECTED;
			private const uint EHR_SP_SERVER_VERSION_NOT_SUPPORTED = ITF_LIC_RNBO_OFFSET+SP_SERVER_VERSION_NOT_SUPPORTED;
			private const uint EHR_SP_FILE_NOT_FOUND = ITF_LIC_RNBO_OFFSET+SP_FILE_NOT_FOUND;
			private const uint EHR_SP_PATH_TOO_LONG = ITF_LIC_RNBO_OFFSET+SP_PATH_TOO_LONG;
			private const uint EHR_SP_SOFT_DB_CORRUPT = ITF_LIC_RNBO_OFFSET+SP_SOFT_DB_CORRUPT;
			private const uint EHR_SP_SOFT_DB_RESTORE_DETECTED = ITF_LIC_RNBO_OFFSET+SP_SOFT_DB_RESTORE_DETECTED;
			private const uint EHR_SP_PRST_DATA_CORRUPT = ITF_LIC_RNBO_OFFSET+SP_PRST_DATA_CORRUPT;
			private const uint EHR_SP_SECURITY_RUNTIME_NOT_DETECTED = ITF_LIC_RNBO_OFFSET+SP_SECURITY_RUNTIME_NOT_DETECTED;
			private const uint EHR_SP_TIME_TAMPER_DETECTED = ITF_LIC_RNBO_OFFSET+SP_TIME_TAMPER_DETECTED;
			private const uint EHR_SP_END_DATE_REACHED = ITF_LIC_RNBO_OFFSET+SP_END_DATE_REACHED;
			private const uint EHR_SP_START_DATE_NOT_REACHED = ITF_LIC_RNBO_OFFSET+SP_START_DATE_NOT_REACHED;
			private const uint EHR_SP_HOST_ID_ERROR = ITF_LIC_RNBO_OFFSET+SP_HOST_ID_ERROR;
			private const uint EHR_SP_LIC_INVALID = ITF_LIC_RNBO_OFFSET+SP_LIC_INVALID;
			private const uint EHR_SP_LIC_RUN_TIME_ERROR = ITF_LIC_RNBO_OFFSET+SP_LIC_RUN_TIME_ERROR;
			private const uint EHR_SP_LIC_MEMORY_CORRUPTED = ITF_LIC_RNBO_OFFSET+SP_LIC_MEMORY_CORRUPTED;
			private const uint EHR_SP_NOT_ENOUGH_MEMORY = ITF_LIC_RNBO_OFFSET+SP_NOT_ENOUGH_MEMORY;
			private const uint EHR_SP_IP_ADDRESS_BLOCKED = ITF_LIC_RNBO_OFFSET+SP_IP_ADDRESS_BLOCKED;
			private const uint EHR_SP_SERVER_OUT_OF_WORKING_TIME = ITF_LIC_RNBO_OFFSET+SP_SERVER_OUT_OF_WORKING_TIME;
			private const uint EHR_SP_SECURITY_RUNTIME_VERSION_MISMATCH = ITF_LIC_RNBO_OFFSET+SP_SECURITY_RUNTIME_VERSION_MISMATCH;
			...
			static CSolLicHRESULT()
			{
				...
				SolLicErrors.Add(EHR_LIC_MAIL_SERVER,"Dynamic Mail Server Error (License server)");
				SolLicErrors.Add(EHR_DONGLE_EMULATOR,"Second 1515 error verifying License Server, please contact Solimar Systems, Inc. (License server)");
				...
				SolLicErrors.Add(EHR_SP_PROTOCOL_NOT_INSTALLED,"The specified protocol is not installed (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_NO_LEASE_FEATURE,"The element does not contain any lease period. Probably, this is not a full licenseelement (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_LEASE_EXPIRED,"The trial version expiration date has reached before the application was first executed (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_COUNTER_LIMIT_REACHED,"The value requested for the decrement operation exceeds the current counter value (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_NO_DIGITAL_SIGNATURE,"The Sentinel driver binary is not signed by a valid authority (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SYS_FILE_CORRUPTED,"The digital certificate of the Sentinel driver is not valid (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_INVALID_DEV_CODE,"You specified an invalid Developer Code (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_DEVID_DOES_NOT_MATCH,"The specified developer ID doesn't match the one in Developer Code (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_DEVICE_SHARING_DETECTED,"Device is shared by more than one system or shared in virtual machine environment (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SERVER_VERSION_NOT_SUPPORTED,"The Sentinel Protection Server from which you are trying to obtain a license is outdated (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_FILE_NOT_FOUND,"Specified file is not found (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_PATH_TOO_LONG,"Specified path is too long (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SOFT_DB_CORRUPT,"The software key database is in corrupt state (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SOFT_DB_RESTORE_DETECTED,"The software key database has been illegally restored to an earlier version (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_PRST_DATA_CORRUPT,"The SuperPro persistence data is in corrupt state (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SECURITY_RUNTIME_NOT_DETECTED,"The Sentinel Security Runtime was not installed or detected (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_TIME_TAMPER_DETECTED,"A time tampering condition is detected because the LKD (Last Known good Date) stored in the key is ahead of the system's current date (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_END_DATE_REACHED,"The software key is invalid because the key end date has reached (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_START_DATE_NOT_REACHED,"The software key is invalid because the system's current date is ahead of the key start date (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_HOST_ID_ERROR,"The fingerprint of the machine has been changed or the license is enforced with the user name and you are using a different user name (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_LIC_INVALID,"SuperPro Software Key is invalid (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_LIC_RUN_TIME_ERROR,"License runtime failed to work due to some error (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_LIC_MEMORY_CORRUPTED,"License is corrupted (like invalid cell format) (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_NOT_ENOUGH_MEMORY,"Not enough storage is available to process this command (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_IP_ADDRESS_BLOCKED,"Cannot communicate with the Sentinel Protection Server because the IP address of the client is blocked by the server (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SERVER_OUT_OF_WORKING_TIME,"Cannot communicate with the Sentinel Protection Server because the server is out of the working time (Rainbow driver)");
				SolLicErrors.Add(EHR_SP_SECURITY_RUNTIME_VERSION_MISMATCH,"SuperPro persistence component versions mismatch (Rainbow driver)");
