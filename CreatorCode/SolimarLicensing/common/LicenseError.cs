...
namespace Solimar
{
	namespace Licensing
	{
		public class CSolLicHRESULT
		{
			...
			private const uint ITF_MIN				= HRESULT_BASE+0x0000;
			...
			private const uint EH_UNKNOWN					= ITF_LIC_SVR_OFFSET + 1;
			private const uint EH_DRIVER_INIT_FAILURE		= ITF_LIC_SVR_OFFSET + 2;
			private const uint EH_PASSWORD_NO_MATCH		= ITF_LIC_SVR_OFFSET + 3;
			private const uint EH_PASSWORD_INVALID_FORMAT	= ITF_LIC_SVR_OFFSET + 4;
			private const uint EH_LICENSE_INSUFFICIENT		= ITF_LIC_SVR_OFFSET + 5;
			private const uint EH_LICENSE_INVALID			= ITF_LIC_SVR_OFFSET + 6;
			private const uint EH_LICENSE_EXPRIED			= ITF_LIC_SVR_OFFSET + 7;
			private const uint EH_KEY_TRIAL_EXPIRED		= ITF_LIC_SVR_OFFSET + 8;
			private const uint EH_KEY_NOT_PRESENT			= ITF_LIC_SVR_OFFSET + 9;
			private const uint EH_CLOCK_ERROR				= ITF_LIC_SVR_OFFSET + 10;
			private const uint EH_KEY_WRITE_FAILURE		= ITF_LIC_SVR_OFFSET + 11;
			private const uint EH_KEY_WRITE_ACCESS_DENIED	= ITF_LIC_SVR_OFFSET + 12;
			private const uint EH_KEY_NOT_ACTIVATED		= ITF_LIC_SVR_OFFSET + 13;
			private const uint EH_AUTHENTICATION_FAILURE	= ITF_LIC_SVR_OFFSET + 14;
			private const uint EH_MANAGER_NOT_INITIALIZED	= ITF_LIC_SVR_OFFSET + 15;
			private const uint EH_KEY_NO_SUITABLE_KEY		= ITF_LIC_SVR_OFFSET + 16;
			private const uint EH_KEY_LOCKED				= ITF_LIC_SVR_OFFSET + 17;
			private const uint EH_CLIENT_TIMEOUT			= ITF_LIC_SVR_OFFSET + 18;
			private const uint EH_PACKET_EXPIRED			= ITF_LIC_SVR_OFFSET + 19;

			// Rainbow driver errors
			private const uint EH_SP_INVALID_FUNCTION_CODE		= ITF_RNBO_OFFSET + SP_INVALID_FUNCTION_CODE;
			private const uint EH_SP_INVALID_PACKET			= ITF_RNBO_OFFSET + SP_INVALID_PACKET;
			private const uint EH_SP_UNIT_NOT_FOUND			= ITF_RNBO_OFFSET + SP_UNIT_NOT_FOUND;
			private const uint EH_SP_ACCESS_DENIED				= ITF_RNBO_OFFSET + SP_ACCESS_DENIED;
			private const uint EH_SP_INVALID_MEMORY_ADDRESS	= ITF_RNBO_OFFSET + SP_INVALID_MEMORY_ADDRESS;
			private const uint EH_SP_INVALID_ACCESS_CODE		= ITF_RNBO_OFFSET + SP_INVALID_ACCESS_CODE;
			private const uint EH_SP_PORT_IS_BUSY				= ITF_RNBO_OFFSET + SP_PORT_IS_BUSY;
			private const uint EH_SP_WRITE_NOT_READY			= ITF_RNBO_OFFSET + SP_WRITE_NOT_READY;
			private const uint EH_SP_NO_PORT_FOUND				= ITF_RNBO_OFFSET + SP_NO_PORT_FOUND;
			private const uint EH_SP_ALREADY_ZERO				= ITF_RNBO_OFFSET + SP_ALREADY_ZERO;
			private const uint EH_SP_DRIVER_NOT_INSTALLED		= ITF_RNBO_OFFSET + SP_DRIVER_NOT_INSTALLED;
			private const uint EH_SP_VERSION_NOT_SUPPORTED		= ITF_RNBO_OFFSET + SP_VERSION_NOT_SUPPORTED;
			private const uint EH_SP_QUERY_TOO_LONG			= ITF_RNBO_OFFSET + SP_QUERY_TOO_LONG;
			private const uint EH_SP_DRIVER_IS_BUSY			= ITF_RNBO_OFFSET + SP_DRIVER_IS_BUSY;
			private const uint EH_SP_PORT_ALLOCATION_FAILURE	= ITF_RNBO_OFFSET + SP_PORT_ALLOCATION_FAILURE;
			private const uint EH_SP_PORT_RELEASE_FAILURE		= ITF_RNBO_OFFSET + SP_PORT_RELEASE_FAILURE;
			private const uint EH_SP_ACQUIRE_PORT_TIMEOUT		= ITF_RNBO_OFFSET + SP_ACQUIRE_PORT_TIMEOUT;
			private const uint EH_SP_SIGNAL_NOT_SUPPORTED		= ITF_RNBO_OFFSET + SP_SIGNAL_NOT_SUPPORTED;
			private const uint EH_SP_INIT_NOT_CALLED			= ITF_RNBO_OFFSET + SP_INIT_NOT_CALLED;
			private const uint EH_SP_FAIL_ON_DRIVER_COMM		= ITF_RNBO_OFFSET + SP_FAIL_ON_DRIVER_COMM;
			private const uint EH_SP_SERVER_PROBABLY_NOT_UP	= ITF_RNBO_OFFSET + SP_SERVER_PROBABLY_NOT_UP;
			private const uint EH_SP_UNKNOWN_HOST				= ITF_RNBO_OFFSET + SP_UNKNOWN_HOST;
			private const uint EH_SP_SENDTO_FAILED				= ITF_RNBO_OFFSET + SP_SENDTO_FAILED;
			private const uint EH_SP_SOCKET_CREATION_FAILED	= ITF_RNBO_OFFSET + SP_SOCKET_CREATION_FAILED;
			private const uint EH_SP_NORESOURCES				= ITF_RNBO_OFFSET + SP_NORESOURCES;
			private const uint EH_SP_BROADCAST_NOT_SUPPORTED	= ITF_RNBO_OFFSET + SP_BROADCAST_NOT_SUPPORTED;
			private const uint EH_SP_BAD_SERVER_MESSAGE		= ITF_RNBO_OFFSET + SP_BAD_SERVER_MESSAGE;
			private const uint EH_SP_NO_SERVER_RUNNING			= ITF_RNBO_OFFSET + SP_NO_SERVER_RUNNING;
			private const uint EH_SP_NO_NETWORK				= ITF_RNBO_OFFSET + SP_NO_NETWORK;
			private const uint EH_SP_NO_SERVER_RESPONSE		= ITF_RNBO_OFFSET + SP_NO_SERVER_RESPONSE;
			private const uint EH_SP_NO_LICENSE_AVAILABLE		= ITF_RNBO_OFFSET + SP_NO_LICENSE_AVAILABLE;
			private const uint EH_SP_INVALID_LICENSE			= ITF_RNBO_OFFSET + SP_INVALID_LICENSE;
			private const uint EH_SP_INVALID_OPERATION			= ITF_RNBO_OFFSET + SP_INVALID_OPERATION;
			private const uint EH_SP_BUFFER_TOO_SMALL			= ITF_RNBO_OFFSET + SP_BUFFER_TOO_SMALL;
			private const uint EH_SP_INTERNAL_ERROR			= ITF_RNBO_OFFSET + SP_INTERNAL_ERROR;
			private const uint EH_SP_PACKET_ALREADY_INITIALIZED= ITF_RNBO_OFFSET + SP_PACKET_ALREADY_INITIALIZED;
			private const uint EH_SP_INVALID_STATUS			= ITF_RNBO_OFFSET + SP_INVALID_STATUS;
