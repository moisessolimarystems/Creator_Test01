...
const SL_ERROR SLErrors[] =
{
	...
	{EC_KEY_BASE_NOT_MATCHING,						EHR_KEY_BASE_NOT_MATCHING,						L"Base Keys do not match module for module, all keys on this server will be ignored (Key server)"},
	{EC_KEY_NOT_MATCHING_INSTANCES,						EHR_KEY_NOT_MATCHING_INSTANCES,						L"Keys do not match by Application Instances for all the Key on the server, all keys on this server will be ignored (Key server)"},
	{EC_KEY_NO_FREE_APP_INSTANCE,						EHR_KEY_NO_FREE_APP_INSTANCE,						L"Unable to obtain an Application Instances on the server, all keys on this server will be ignored (Key server)"},
