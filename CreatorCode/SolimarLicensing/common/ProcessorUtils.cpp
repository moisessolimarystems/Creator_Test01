...
DWORD ProcessorUtils::GetProcessorLicenseCount()
{
	...
	if (Initialize())
	{
		licensedProcessors = m_numAvailablePackages * ceil( coreFactor * ( double(m_numAvailableCores) / double(m_numAvailablePackages)));
	}
										...
										DWORD& numAvailableCores)
{
	...
	numAvailablePackages = pkgIDSet.size();
	numAvailableCores = coreIDSet.size();
