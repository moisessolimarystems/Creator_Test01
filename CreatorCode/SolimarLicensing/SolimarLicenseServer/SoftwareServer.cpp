...
HRESULT SoftwareServer::GetSoftwareLicenseInfo_ByLicense(BSTR softwareLicense, BSTR *pBstrLicenseInfoAttribsStream)
{
	...
	if(SUCCEEDED(hr))
	{
		...
		//Force refresh of cache if clock violation changed, on license info object it is only for display...
		if(swLicClockViolationMap.find(softwareLicense) == swLicClockViolationMap.end() || swLicClockViolationMap[softwareLicense] != licServerDataMgr.InClockViolation())
		{
			bForceRefresh = true;
			swLicClockViolationMap[softwareLicense] = licServerDataMgr.InClockViolation();
		}

		//Update Cache if the Software License is not in cache, or the modified date is different
		if(bForceRefresh || 
			swLicStreamedCacheMap.find(softwareLicense) == swLicStreamedCacheMap.end() || 
...
OutputDebugString(L"SoftwareServer::GenerateLicenseSystemData() - Get System Info - end");
		{
			...
			if(SUCCEEDED(hr))
			{
				...
				hr = wmi.ExecuteQuery((wchar_t*)wmiQuery_DomainByte, DomainList);
				if(SUCCEEDED(hr) && DomainList.size()>0)
				{
					for (unsigned int idx=0; idx<DomainList.size(); ++idx)
					{
						...
						VARIANT* pvtDomain = &DomainList[idx][(wchar_t*)domainByte];
						if (pvtDomain->vt == VT_BSTR)
						{
							sysInfoAttribs.domainNameList->push_back(std::wstring(pvtDomain->bstrVal));
						}
					}
					WMIHelper::Clear(DomainList);
					DomainList.clear();
				}
			}
			if(SUCCEEDED(hr))
			{
				// gather system manufacturer information
				WMIHelper::ResultsType rList;
				...
				if(SUCCEEDED(hr) && rList.size()>0)
				{
					for (unsigned int idx=0; idx<rList.size(); ++idx)
					{
						...
						VARIANT* pvtTmp = &rList[idx][(wchar_t*)tmpByte];
						if (pvtTmp->vt == VT_BSTR)
							sysInfoAttribs.systemManufacturerList->push_back(std::wstring(pvtTmp->bstrVal));
					}
					WMIHelper::Clear(rList);
					rList.clear();
				}
			...
			if(SUCCEEDED(hr))
			{
				// gather system model information
				WMIHelper::ResultsType rList;
				...
				if(SUCCEEDED(hr) && rList.size()>0)
				{
					for (unsigned int idx=0; idx<rList.size(); ++idx)
					{
						...
						VARIANT* pvtTmp = &rList[idx][(wchar_t*)tmpByte];
						if (pvtTmp->vt == VT_BSTR)
							sysInfoAttribs.systemModelList->push_back(std::wstring(pvtTmp->bstrVal));
					}
					WMIHelper::Clear(rList);
					rList.clear();
				}
			...
			if(SUCCEEDED(hr))
			{
				// gather system type information
				WMIHelper::ResultsType rList;
				...
				if(SUCCEEDED(hr) && rList.size()>0)
				{
					for (unsigned int idx=0; idx<rList.size(); ++idx)
					{
						...
						VARIANT* pvtTmp = &rList[idx][(wchar_t*)tmpByte];
						if (pvtTmp->vt == VT_BSTR)
							sysInfoAttribs.systemTypeList->push_back(std::wstring(pvtTmp->bstrVal));
					}
					WMIHelper::Clear(rList);
					rList.clear();
				}
			...
			if(SUCCEEDED(hr))
			{
				...
				if(SUCCEEDED(hr) && rList.size()>0)
				{
					for (unsigned int idx=0; idx<rList.size(); ++idx)
					{
						...
						VARIANT* pvtTmp = &rList[idx][(wchar_t*)tmpByte];
						if (pvtTmp->vt == VT_BSTR)
							sysInfoAttribs.systemUuidList->push_back(std::wstring(pvtTmp->bstrVal));
					}
					WMIHelper::Clear(rList);
					rList.clear();
				}
...
HRESULT SoftwareServer::GenerateSoftwareLicArchive_ByLicense(BSTR softwareLicense, VARIANT* pVtLicArchive)
{
	...
	try
	{
		...
		// Ensure that not in time clock violation right now...
...
HRESULT SoftwareServer::EnterSoftwareLicArchive(VARIANT vtLicArchive)
{
	...
	try
	{
		...
		if(pSoftwareLicMgr == NULL)
		{
			...
			BYTE fileLocByte[] = {
				0x53, 0x00, 0x6f, 0x00, 0x6c, 0x00, 0x69, 0x00, 0x6d, 0x00, 0x61, 0x00, 0x72, 0x00, 0x5c, 0x00, 0x5c, 0x00, 0x44, 0x00, 0x61, 0x00, 0x74, 0x00, 0x61, 0x00, 0x5c, 0x00, 0x5c, 0x00, 0x00, 0x00
			};
...
HRESULT SoftwareServer::ApplyLicensePacketInternal(BSTR bstrLicPackageAttribsStream, _bstr_t bstrVerificationCode)
{
	...
	try
	{
		...
		if(!TimeHelper::StringToSystemTime(std::wstring(SpdAttribs::WStringObj(tmpLicPackageAttribs.licLicenseInfoAttribs.modifiedDate)).c_str(), packetCreateDateSystime))
			throw(E_FAIL);
		...
		if ((tmpLicPackageAttribs.licSoftwareSpecAttribs.productSpecMap->size() == 0)
			&& ((int)g_pSoftwareSpec->GetSoftwareSpec().softwareSpec_SubMinor < (int)tmpLicPackageAttribs.licSoftwareSpecAttribs.softwareSpec_SubMinor))
		{
