	...
	: TForm(Owner)
{
    ...
    //Create a string list to hold packet names
    PktList = new TStringList();
    //
    ...
    resetKey(SOLSEARCHER_ENTERPRISE_PRODUCT);
    //
...
void __fastcall TFCustomerKeys::showLicenseInfo(bool bTurnOn)
{
    ...
    if(!Licensing->Visible)
        //Move App Instance GroupBox into Position #1
        AppInstanceGroupBox->Left = Licensing->Left;
    else
        AppInstanceGroupBox->Left = (Licensing->Left + Licensing->Width + 25);
        ...
        if(QueryKey->Active)
        {
            if(QueryKey->RecordCount)
            {
                ...
                else if( key_record->pkey->productId==SOLSEARCHER_ENTERPRISE_PRODUCT)
                {
                       ...
                       ((SSProtectionKey*)(key_record->pkey))->setLicenses((unsigned short*)buffer);   //need to process 32 bytes of binary modules                                        //
                ...
                else if( key_record->pkey->productId==SPDE_PRODUCT)
                {
                        ...
                        //memset(tmp_spde_modules, 0, sizeof(tmp_spde_modules));  //array of 16 shorts = 32 bytes
                        QueryKey->Fields->FieldByName("modules")->GetData(buffer, true);  // gives me 32 bytes of binary modules
                       ((SpdeProtectionKey*)(key_record->pkey))->setLicenses((unsigned short*)buffer);   //need to process 32 bytes of binary modules

                        ((SpdeProtectionKey*)(key_record->pkey))->setOutputUnits(QueryKey->FieldByName("SKRoutput")->AsInteger);
                        ((SpdeProtectionKey*)(key_record->pkey))->setOperatorSessionUnits(QueryKey->FieldByName("SKRoperatorSession")->AsInteger);
                        ((SpdeProtectionKey*)(key_record->pkey))->setUserSessionUnits(QueryKey->FieldByName("SKRuserSession")->AsInteger);
                        // initialize module tab and make it visible
                        KeyFormModuleFrame->initialize(MODE_2, key_record->pkey->productId, modulePasswordCreated);
                        PageControl1->Pages[1]->TabVisible = true;
                        PageControl1->Pages[2]->TabVisible = false;
...
void __fastcall TFCustomerKeys::setKeyInfoValues()
{
    ...
    //initialize Application Instance EditBox
    setApplicationInstanceUnitsDisplay(key_record->getAppInstances());
    ...
    else if (key_record->pkey->productId == SPDE_PRODUCT)
    {
        ...
        //
        // LICENSING - refresh licensing.
        setOutputUnitsDisplay(static_cast<SpdeProtectionKey*>(key_record->pkey)->getOutputUnits());
        setOperatorSessionUnitsDisplay(static_cast<SpdeProtectionKey*>(key_record->pkey)->getOperatorSessionUnits());
        setUserSessionUnitsDisplay(static_cast<SpdeProtectionKey*>(key_record->pkey)->getUserSessionUnits());
    }
    else if (key_record->pkey->productId == SOLSCRIPT_PRODUCT ||
             key_record->pkey->productId == RUBIKA_PRODUCT ||
             key_record->pkey->productId == SDX_DESIGNER_PRODUCT ||
	...
	const TRect &Rect, int DataCol, TColumn *Column, TGridDrawState State)
{
   ...
   else if (Column->FieldName == "SKRappInstance")
   {
       KeyGrid->Canvas->FillRect( Rect );
   ...
   switch( _index )
   {
      ...
      case 1 : //module page refresh
                 ...
                 (key_record->pkey->productId == ICONVERT_PRODUCT)  )
            {
               ...
               setOutputUnitsDisplay(static_cast<SpdProtectionKey*>(key_record->pkey)->getOutputUnits());
            }
            else if ( key_record->pkey->productId == SPDE_PRODUCT)
            {
...
{
   operator_sessions->Text = (static_cast<SpdeProtectionKey*>(key_record->pkey)->getOperatorSessionUnits());;
}

void TFCustomerKeys::setUserSessionUnitsDisplay(unsigned short units)
{
   user_sessions->Text = (static_cast<SpdeProtectionKey*>(key_record->pkey)->getUserSessionUnits());;
}

void TFCustomerKeys::setApplicationInstanceUnitsDisplay(unsigned short units)
{
   AppInstanceEdit->Text = key_record->getAppInstances();
}
...
void TFCustomerKeys::createExtensionPackets(unsigned short days)
{
   TBookmark tmpMark;
   AnsiString AnsiPassword;            //stores password read from grid
   AnsiString AnsiDescription;         //stores description read from grid
...
void TFCustomerKeys::createPasswordPackets(unsigned short days)
{
   TBookmark tmpMark;
   AnsiString AnsiPassword;            //stores password read from grid
   AnsiString AnsiDescription;         //stores description read from grid
   ...
   GetLocalTime(&CurrentTime);
   SystemTimeToVariantTime(&CurrentTime, &varTime.date);
   varTime.date += days;      //add 2 years to the expiration date.
...
{
   AnsiString packetList;
   int daysExpire;
   //get number of days till packet expires
   ...
   createExtensionPackets(daysExpire);
   createPasswordPackets(daysExpire);

   for(int i=0; i < PktList->Count;i++)
   {
   ...
   Application->MessageBox(packetList.c_str(),"Password Packet(s) Created : ",MB_OK);
   PktList->Clear();
}
...
bool TFCustomerKeys::dbSaveKey(SKeyRecord* programmed, TKeyWizardFrm* wizard)
{
   ...
   try
   {
      ...
      if      (selectedProduct == SPD_PRODUCT       ||
         ...
         output = reinterpret_cast<SpdeProtectionKey*>(programmed->pkey)->getOutputUnits();
         operatorSessions =   reinterpret_cast<SpdeProtectionKey*>(programmed->pkey)->getOperatorSessionUnits();
         userSessions =    reinterpret_cast<SpdeProtectionKey*>(programmed->pkey)->getUserSessionUnits();
      }
      else {
...
//-----------------------------------------------------------
      ...
      UtilityQuery->ParamByName("appinstance")->AsInteger = programmed->getAppInstances();//appInstance      
...
void TFCustomerKeys::createOutputPassword(int output_units)
{
   ...
   try
   {
      ...
      UtilityQuery->ParamByName("module_list")->AsBlob = updated_module_list;
      UtilityQuery->ParamByName("output_units")->AsInteger = spd_key->getOutputUnits();
      UtilityQuery->ParamByName("keyId")->AsInteger = key_record->skr_id;
      UtilityQuery->ExecSQL();
      ...
      UtilityQuery->ParamByName("skr_id")->AsInteger = key_record->skr_id;
      UtilityQuery->ParamByName("value")->AsInteger = key_record->getPasswordNumber();
      UtilityQuery->Prepare();
      UtilityQuery->ExecSQL();
...
void __fastcall TFCustomerKeys::KeyGridTitleClick(TColumn *Column)
{
    ...
    if (Column->FieldName == "SCRnumber")
        ...
        my_sql_statement += key_sort_type[SORT_APPINSTANCE];
...
void __fastcall TFCustomerKeys::KeyFormModuleFramemmSetModuleClick(
      TObject *Sender)
{
    KeyFormModuleFrame->mmSetModuleClick(Sender);
...
//==============================================================================
// Function:    createAppInstancePassword()
// Purpose:     This function creates a multiple password for a user for a certain
//              key.  This function updates the SKeyRecord and STransactionDetail
//              tables in the database.
// Parameters:  unsigned short - version
// Returns:     None
//==============================================================================
void TFCustomerKeys::createAppInstancePassword(unsigned short appInstances, ushort password_number)
{
   char password_string[128];
   ...
   //
   //generate password
   key_record->setAppInstances(appInstances);
   keyMaster->getAppInstancePassword(key_record->pkey, key_record->getAppInstances(), password_number, password_string);
   if(!password_string)
   {
      //Application->MessageBox("Unable to generate password.", "Key Message", MB_OK|MB_ICONERROR );
      return;
   ...
   //
   //Update database
   //add new column in database for multiple
   try
   {
      Database1->StartTransaction();
      ...
      UtilityQuery->Close();
      UtilityQuery->SQL->Clear();
      UtilityQuery->SQL->Add("UPDATE SKeyRecord SET SKRappInstance = :appInstances, SKRpasswordNumber = :passwordNumber WHERE SKRid = :keyId ");
      UtilityQuery->ParamByName("keyId")->AsInteger = key_record->skr_id;
      UtilityQuery->ParamByName("appInstances")->AsInteger = key_record->getAppInstances();
      UtilityQuery->ParamByName("passwordNumber")->AsInteger = key_record->getPasswordNumber();
      UtilityQuery->ExecSQL();

      UtilityQuery->SQL->Add("INSERT INTO sTransactionDetail (SKRid, TDpassword, SDRid, TDunits, TDrow_id) values (:key_id, :password, 256, :units, 0)");
      UtilityQuery->ParamByName("key_id")->AsInteger = key_record->skr_id;
      UtilityQuery->ParamByName("units")->AsString = key_record->getAppInstances();
      UtilityQuery->ParamByName("password")->AsString = password_string;
      UtilityQuery->ExecSQL();
...
void __fastcall TFCustomerKeys::mmApplicationInstanceClick(TObject *Sender)
{
    //open set module dialog, store value in database, call licenseserver function
   TUnitsDlg *pDlg = new TUnitsDlg(NULL, 100);  //set application instances max at 100
   pDlg->Edit1->Text = key_record->getAppInstances();
   if (pDlg->ShowModal()==IDYES)
      createAppInstancePassword(pDlg->GetUnits(),key_record->incrementPasswordNumber());
   delete pDlg;
