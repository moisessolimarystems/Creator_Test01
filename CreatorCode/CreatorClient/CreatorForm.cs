...
namespace Client.Creator
{
    public partial class CreatorForm : Form
    {
        ...
        private void ResetMainToolStripMenu()
        {
            ...
            //searchToolStripTextBox.Visible = SearchToolStripLabel.Visible = false;
            printToolStripButton.Enabled = true; //print button always available
        }

        //context detailtreeview node has been selected. All necessary context info should be available from the node.
        //CONTACT DB : 4
        ...
        LicenseServer GetLicenseServerFromSelectedNode(TreeNode node)
        {
            TreeNode currentNode = node;
            LicenseServer ls = null;
            if (currentNode != null)
            {
                while (!(currentNode.Tag is LicenseServer))
                {
                    if (currentNode == null)
                        break;
                    currentNode = currentNode.Parent;
                }
                ls = currentNode.Tag as LicenseServer;
            }
            return ls;
        }
        ...
        private void EnableToolStripMenu()
        {            
            ResetMainToolStripMenu();
            searchToolStripTextBox.Visible = SearchToolStripLabel.Visible = viewToolStripComboBox.SelectedIndex != AppConstants.ReportsView;
            searchToolStripTextBox.Enabled = SearchToolStripLabel.Enabled = viewToolStripComboBox.SelectedIndex != AppConstants.ReportsView;
            if (searchToolStripTextBox.Text.Length == 0 && !searchToolStripTextBox.Focused)
            {
                searchToolStripTextBox.Text = (viewToolStripComboBox.SelectedIndex == AppConstants.CustomersView || viewToolStripComboBox.SelectedIndex == AppConstants.TokensView) ? "Customer Filter" : "License Server Filter";
            ...
            viewToolStripComboBox.Enabled = true;
            if (viewToolStripComboBox.SelectedIndex == AppConstants.CustomersView)
            {
                newCustMainToolStripBtn.Enabled = m_Permissions.pt_create_modify_key.Value;
                customerToolStripStatusLabel.Text = string.Empty;
            }
            else if (viewToolStripComboBox.SelectedIndex == AppConstants.LicensesView)
            {
                ...
                if (DetailTreeView.SelectedNode != null)
                {
                    if (DetailTreeView.SelectedNode.Tag is LicenseServer)
                    {
                        ...
                        if (lsp.UserLock != string.Empty)
                            addProductLicenseMainToolStripDropDownBtn.Enabled = lsp.LockedByCurrentUser() &&
                                                                                lsp.IsActive &&
        ...
        private void navigateHomeToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            {   //load customer view
                viewToolStripComboBox.SelectedIndex = -1;
                viewToolStripComboBox.SelectedIndex = AppConstants.TokensView;
            }
        }
            ...
            if (MainTabControl.SelectedTab != null)
            {
                ...
                switch (MainTabControl.SelectedTab.Name)
                {
                    ...
                    case AppConstants.ReportTabPage:
                        LoadReportListView();
                        reportsSplitContainer.Parent = splitContainer2.Panel1;
                        break;
                    default: break;
                }
                EnableToolStripMenu();
        ...
        private void checkInLicenseServer(TreeNode LicenseServerNode)
        {
            if (LicenseServerNode.Tag != null)
            {
                ...
                PopulateDetailListView(LicenseServerNode.Tag);
            }
        }
        ...
        {         
                                ...
                                MessageBoxIcon.Exclamation) == DialogResult.OK)
            {
                Service<ICreator>.Use((client) =>
                {
                    ...
                    licRecord.IsDirty = true;
                    client.UpdateLicense(licRecord); //DB-1
                ...
                PopulateDetailListView(DetailTreeView.SelectedNode.Tag);
                ...
                if (cd.ShowDialog(this, data) == DialogResult.OK)
                {
                    try
                    {
                        ...
                        Service<ICreator>.Use((client) =>
                        {
                            ...
                            s_AllSoftwareTokens = client.GetAllSoftwareTokens();
                        });
                        viewToolStripComboBox.SelectedIndex = AppConstants.CustomersView;
                    }
        ...
        private void SearchCurrentView(string searchString)
        {
            if (searchToolStripTextBox.ForeColor == SystemColors.InactiveCaptionText)
                searchToolStripTextBox.Text = string.Empty;
            if (MainTabControl.SelectedTab == CustomersTabPage)            
        ...
        private void LoadSelectedLicenseServer(string selectedLicense)
        {
            ...
            Service<ICreator>.Use((client) =>
            {
                if (client.GetLicenseByName(selectedLicense, false) == null)
                {
                    MessageBox.Show("Could not find License Server : " + selectedLicense, "Search Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    bFound = false;
                }
                else
        ...
        TreeNode GetLicenseNode(TreeNode currentNode)
        {
        ...
        private void SetLicenseServerState(TreeNode currentNode, bool bAllLicenseServers)
        {
            ...
            TreeNode licenseNode = GetLicenseNode(currentNode);
            ...
            {
                if (!bAllLicenseServers && node.Name != licenseNode.Name)
        ...
        private string GetLicenseNameFromTreeView()
        {
            ...
            LicenseServer ls = GetLicenseServerFromSelectedNode(DetailTreeView.SelectedNode);
            if (ls != null)
                licenseName = ls.Name;
            /*if (DetailTreeView.SelectedNode.Tag is LicenseServer)
        ...
        {
            ProductLicenseTable plTable = null;
            ...
            using (LicenseInfoForm dlg = new LicenseInfoForm("New Product License(s) for " + licenseName, ref s_CommLink))
            {
                ...
                if (dlg.ShowDialog(this, data) == DialogResult.OK)
                {
                    ...
                    {
                        plTable = plt;
                        productName = s_CommLink.GetProductName(plTable.ProductID); 
                        if (!DetailTreeView.SelectedNode.IsExpanded) //expand to search for product node
                        ...
                        if (productNode == null) //create the product node to attach the product license node
                        {
                        ...
                        if (productLicenseNode == null)
                        {
                            ...
                            {   //add to parent order node
                                parentProductLicenseNode = DetailTreeView.SelectedNode.Nodes.Find(plTable.ParentProductLicenseID, true).FirstOrDefault();
        ...
        private void LoadPacketItems(string licenseName)
        {
            ...
            if (packets != null && packets.Count > 0)
            {
                foreach (var packet in packets)
                {
                    ...
                    {
                        packet.PacketName,
                        packet.ExpiredDate.ToShortDateString(),
                        packet.UserName,
                        packet.VerifiedBy ?? string.Empty
                    };
        ...
        private void EnableHardwareKeyView()
        {
            ...
            SearchToolStripLabel.Visible = searchToolStripTextBox.Visible = false;
            if (HardwareKeyListView.SelectedItems.Count > 0)
            {
        ...
        #region DetailListView Methods
            ...
            public void SetupDetailListViewToolStrip(Object currentObject)
            {
                ...
                {
                    ProductCollection productData = currentObject as ProductCollection;
                    imageIndex = Enums.GetIconIndex(s_CommLink.GetProductBaseName((byte)productData.ProductID));
                    toolStripText = string.Format("{0} Module Summary", s_CommLink.GetProductName(productData.ProductID));
                }
                else //Product License
                {
                    ProductLicense plData = currentObject as ProductLicense;
                    imageIndex = Enums.GetIconIndex(s_CommLink.GetProductBaseName(plData.ProductName));
                    toolStripText = string.Format("{0} Modules", plData.ProductName);
                }
                ...
                DetailListViewToolStripLabel.Text = toolStripText;
                DetailListViewToolStripLabel2.Visible = currentObject is ProductCollection;// false;
                //enable/disable buttons
                dlvAddToolStripButton.Visible = currentObject is LicenseServer; //true;
                dlvRemoveToolStripButton.Visible = currentObject is LicenseServer;// true;
                moduleFilterToolStripComboBox.Visible = currentObject is ProductLicense;// false;
                totalModuleListToolStripButton.Visible = currentObject is ProductLicense;// false;
                defaultModuleListToolStripButton.Visible = currentObject is ProductLicense;// false;
            }
            ...
            public void PopulateDetailListView(LicenseServer licenseData)
            {
                ...
                DetailListView.Items.Clear();
                SetupDetailListViewToolStrip(licenseData);
                ListViewItem lvItem;        
               ...
               // retrieve tokens from database not license info object
                ...
                {                    
                    ...
                    swTokenList = s_AllSoftwareTokens;
                });
                if (tokenList != null)
                {             
                    ...
                    bool bLockStatus = licenseData.LockStatus; //db - 2 refreshes lockstatus
            ...
            public void PopulateDetailListView(ProductCollection productData)
            {
                ...
                List<ModuleTable> moduleList = null;
                DetailListView.Items.Clear();
                SetupDetailListViewToolStrip(productData);
                //needs to skip any product,modules that are deactivated            
                //Need function to get correct parent node
                LicenseServer lsp = GetLicenseServerFromSelectedNode(DetailTreeView.SelectedNode); //DetailTreeView.SelectedNode.Parent.Tag as LicenseServer;
                Service<ICreator>.Use((client) =>
                {
                ...
                foreach (var packet in packets)
                {
                    ...
                    if (selectedTransactions.Count > 0)
                    {
                        ...
                        for (index = 0; index < selectedTransactions.Count; index++)
                        {
                            ...
                            subItems = new string[]
                            {
                                Enum.GetName(typeof(TransactionType), selectedTransactions[index].taType),
                                selectedTransactions[index].taDescription,
                                selectedTransactions[index].taUnits,
                                selectedTransactions[index].taPreviousValue,
                                productLicenseName,
                                selectedTransactions[index].taUser,
                                packet.PacketName
                            };
                            item.SubItems.AddRange(subItems);
                            lvItems[index] = item;
            ...
            if (licenses != null)
            {
                ...
                for (int index = 0; index < licenses.Count; index++)
                {
                    ...
                    if (licenses[index].ExpirationDate.HasValue)
                        ...
                        newItem.SubItems.Add(string.Empty);
                    newItem.SubItems.Add(ProductLicense.GetProductLicenseType(licenses[index].TypeID));
        ...
        public void PopulateReportListViewColumns(Report.ReportType _reportType)
        {
            ...
            {   //values loaded in column order
                ...
                case Report.ReportType.ProductLicense:
                    ...
                    ReportListView.Columns.Add("Type");
        ...
        private Rectangle NodeBounds(TreeNode node)
        {
            ...
            if (node.Tag != null)
            {
                ...
                int tagWidth = (int)g.MeasureString
                    (node.Tag.ToString(), /*tagFont*/this.Font).Width + 6;
        ...
        private string CurrentNodeText(object currentObject)
        {
            string retVal = string.Empty;
            if (currentObject is LicenseServer)
            {
                retVal = (currentObject as LicenseServer).Name;
            }
            else if (currentObject is ProductCollection)
            {
                retVal = s_CommLink.GetProductName((currentObject as ProductCollection).ProductID);
            }
            else if (currentObject is ProductLicense)
            {
                retVal = (currentObject as ProductLicense).ID;
            }
            return retVal;
        }
        ...
        private void DetailTreeView_BeforeSelect(object sender, TreeViewCancelEventArgs e)
        {
            //if(DetailTreeView.SelectedImageIndex >= 0) //selectedimageindex = -1 when nothing application is closing.
            //    LoadDetailTreeViewSelectedNode(e.Node);            
        }
        ...
        {
            MainTabControl.SelectedTab = null;
            //expand
            if (viewToolStripComboBox.SelectedIndex == AppConstants.CustomersView)
            {
                MainTabControl.SelectedTab = CustomersTabPage;
            }
            else if (viewToolStripComboBox.SelectedIndex == AppConstants.LicensesView)
            {
            ...
            {
                MainTabControl.SelectedTab = ValidationKeysTabPage;
            }
            else //reports
            {
        ...
        void SetNodeStyle(TreeNode node, ProductLicenseStatus status)
        {
            switch (status)
            {
                case ProductLicenseStatus.Default:
