    ...
    public partial class CreatorForm : Form
    {
        ...
        private void EnableProductLicenseContextMenu(bool value)
        {
            ...
            if (DetailTreeView.SelectedNode.Tag is ProductLicenseProperty)
            {
                ...
                if (plProperty.LicenseServer.Contains("F"))
                    ...
                    if (!(DetailPropertyGrid.SelectedObject as LicenseServerProperty).IsDeactivated)
                    {
                        ...
                        if (m_Permissions.pt_module_pwd.Value)
                        {
                           ...
                           // {   //enable edit/delete menu items for item selected
                                ...
                                dlvDeleteToolStripMenuItem.Enabled = (DetailListView.Items.Count >= 1) ? true : false;
                ...
                switch (DetailTreeView.SelectedNode.Level)
                {
                    ...
                    case 2:
                        ...
                        if (plSelected.Status == ProductLicenseState.Deactivated)
                            DetailPropertyGrid.Enabled = false;
                        LoadTransactionItems(plSelected.LicenseServer);
                        break;
                    case 3:
                        //Add-on plData level         
            ...
            if (deleteToolStripMenuItem.Text == "Deactivate")
            {
                ...
                if (bDelete)
                {
                    if (DetailTreeView.SelectedNode.Tag is LicenseServerProperty)
                    {
                        ...
                        List<string> lsDeactivate = new List<string>();
                        Service<ICreator>.Use((client) =>
                        {
                            if (selectedLicense.IsStandardLicenseType)                              
                                lsDeactivate = client.GetDerivedLicenseNames(selectedLicense.CustID, selectedLicense.DestID, selectedLicense.GroupID, (byte)selectedLicense.SoftwareLicenseType);
                            lsDeactivate.Add(selectedLicense.Name);
                            foreach (string ls in lsDeactivate)
                            {
                                LicenseServerProperty lsProperty = DetailTreeView.Nodes.Find(ls,true)[0].Tag as LicenseServerProperty;
                                CreateTransaction(TransactionType.LicenseServer,
                                                  lsProperty.Name,
                                                  string.Format("Changed License Server Status from {0} to {1}", lsProperty.LicType.ToString(), LicenseServerType.Deactivated.ToString()),
                                                  lsProperty.LicType.ToString());
                        ...
                        {   //decrement app instance and modules from product view
                            ...
                            if (plAddOns.Count > 0)
                            {
                                ...
                                foreach (string pl in plAddOns)
                                {
                                    ...
                                    // //plt.plID.Substring(0, plt.plID.LastIndexOf("-")),
                                    CreateProductLicenseTransaction(TransactionType.Product,
                                                                    plt.plID,                                                     
                                                      string.Format("{0} : Changed Product License Status from {1} to {2}", 
                                                                    plt.plID,
                                                                    Enum.GetName(typeof(ProductLicenseState),plt.plState), 
                                                                    ProductLicenseState.Deactivated.ToString()),
                                                      ProductLicenseState.Deactivated.ToString());
        ...
        private void dlvRemoveToken(LicenseServerProperty selectedLicense)
        {
            ...
            {
                if(bHardwareToken)
                    CreateTransaction(TransactionType.LicenseServer, selectedLicense.Name,"Removed Hardware Token", "Hardware");
                else
                    CreateTransaction(TransactionType.LicenseServer, selectedLicense.Name, "Removed Software Tokens", "Software");
        ...
        private void dlvRemoveModule(ProductLicenseProperty selectedOrder)
        {
            ...
            foreach (ListViewItem selectedItem in DetailListView.SelectedItems)
            {
                ...
                CreateProductLicenseTransaction(TransactionType.Product, selectedOrder.ID, string.Format("Removed Module - {0}", s_CommLink.GetModuleName(selectedOrder.Product.ID, module.ID)), module.Units.ToString());
            }
            SaveLicense();
        }
        ...
        private void verifyToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {   //selectedItem text will be packet name
                ...
                if (dlg.ShowDialog(this, data) == DialogResult.OK)
                {
                    ...
                    {
                        //create transaction
                        CreateTransaction(TransactionType.LicenseServer, data.LicenseName, string.Format("Verified packet - {0}", data.SelectedPacketName), data.SelectedPacketName);
        ...
        public void CreateNewValidationToken(LicenseServerProperty licInfo)
        {
            using (LicenseInfoForm dlg = new LicenseInfoForm("Add Validation Tokens", ref s_CommLink))
            {
                ...
                if (dlg.ShowDialog(this, data) == DialogResult.OK)
                {
                    ...
                    string transactionMsg = (bHardwareToken) ? "Add Hardware Token" : "Add Software Tokens";
                    CreateTransaction(TransactionType.LicenseServer, licInfo.Name, "Added Tokens", transactionMsg);              
                    SaveLicense();
                    PopulateDetailListView(data.LicInfo);
                }
            }
                ...
                foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs dbProduct in licPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                {
                    if (dbProduct.productID.TVal.Equals(plData.Product.ID))
                    {
                        ...
                        if(plData.ProductLicenseType != Lic_PackageAttribs.Lic_ProductSoftwareSpecAttribs.TProductLicenseType.pltClient)
                            if (plData.Status != ProductLicenseState.AddOn && plData.Status != ProductLicenseState.Deactivated)
        ...
        void AddExistingProductLicense(LicenseServerProperty licData, List<string> productLicenses)
        {
            ...
            Service<ICreator>.Use((client) => 
            {
                ...
                LicenseTable baseLicRec = client.GetLicenseByName(licenseName, false);//string.Format("{0:x4}-{1:x3}-{2:x4}-{3}01", licData.CustID, licData.DestID, licData.GroupID, licenseBase), false);
                if (baseLicRec == null) //subscription type
                {
                    ...
                    baseLicRec = client.GetLicenseByName(licenseName, false);//string.Format("{0:x4}-{1:x3}-{2:x4}-{3}01", licData.CustID, licData.DestID, licData.GroupID, licenseBase), false);
                }
                if (baseLicRec == null)
                {
                ...
                foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs product in baseLicPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                {
                    ...
                    {                        //sub-orders                         
                        ...
                        {                            //get standard order
                            ...
                            CreateTransaction(TransactionType.ProductLicense,
                                              licData.Name,
                                              string.Format("Added {0} Product License - {1}", s_CommLink.GetProductName((uint)orderRec.ProductID), Enum.GetName(typeof(ProductLicenseState), orderRec.plState)),
                                              orderRec.plID);
        ...
        void UpdateExistingProductLicense(LicenseServerProperty licData)
        {
            ...
            Service<ICreator>.Use((client) => 
            {
                ...
                //remove any dr orders that don't exist in std or are deactivated
                ProductLicenseProperty plp;
                ...
                {
                    plNumbers = Lic_LicenseInfoAttribsHelper.GetContractNumbers(subLicProduct);
                    ...
                    {   //find standard order
                        ...
                        foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs baseProduct in baseLicPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                        {
                            ...
                            if (productID == subLicProduct.productID.TVal)
                            {
                                splNumbers = Lic_LicenseInfoAttribsHelper.GetContractNumbers(baseProduct);
                                currProductLicense = plNumber.Replace(replaceString, licenseBase);
                                plRec = client.GetProductLicense(currProductLicense);
                                if (splNumbers.Contains(currProductLicense) && plRec.plState != (byte)ProductLicenseState.Deactivated)                                
                                    bFound = true;
                                break;
                            }
                        }
                        ...
                        {  //remove order from database
                            ...
                            ProductLicenseTable pltDelete = client.GetProductLicense(plNumber);
                            if (pltDelete != null)
                                //    client.DeleteProductLicense(pltDelete);
                                DeleteOrderEntity(pltDelete);
                        }
                    }
                }
                bool bUpdated = false;
                    ...
                    {   //stdLicense Order
                        ...
                        if (plRec != null)
                        {
                            ...
                            {   //subLicense Order
                                ...
                                if (dbplRec == null)
                                {
                                    //remove product/modules
                                    //remove all deactivated modules
                                    ...
                                    {
                                        if (product.productID.TVal == productID)
                                        {
                                            Lic_PackageAttribs.Lic_ProductInfoAttribs.Lic_ModuleInfoAttribsList moduleList = Lic_LicenseInfoAttribsHelper.GetModuleList_ByContractNumber(product, string.Format("{0}-{1}", licData.Name, plRec.plIndex));
                                            foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs mod in moduleList.TVal)
                                            {
                                                product.moduleList.TVal.Remove(mod);
                                            }
                                            if (product.moduleList.TVal.Count == 0)
                                ...
                                {//stored order exists, update - only update state                                                                      
                                    ...
                                    foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs product in subLicPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                                    {
                                        ...
                                        {
                                            product.product_Major.TVal = stdProduct.product_Major.TVal;
                                            product.product_Minor.TVal = stdProduct.product_Minor.TVal;
                                            product.product_SubMajor.TVal = stdProduct.product_SubMajor.TVal;
                                            product.product_SubMinor.TVal = stdProduct.product_SubMinor.TVal;  
                ...
                if (bUpdated)
                {
                    foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs removeProduct in removeProducts)
                    {
                        subLicPackage.licLicenseInfoAttribs.TVal.productList.TVal.Remove(removeProduct);
                    }
        ...
        //ISSUE TODO : product node still exists after resync deactivated pl
        private void FailoverUpdateProductLicense(LicenseServerProperty licData)
        {   //licdata = Failover License Data
            Service<ICreator>.Use((client) => 
            {
                string licenseBase = "P";
                string licenseName = Lic_LicenseInfoAttribsHelper.GenerateLicenseServerName((int)licData.CustID, (int)licData.DestID, (int)licData.GroupID, Lic_PackageAttribs.Lic_LicenseInfoAttribs.TSoftwareLicenseType.sltPerpetual, 1);
                ...
                ProductLicenseTable splRec, fplRec;
                    ...
                    {   //find standard order
                        ...
                        foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs baseProduct in licPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                        {
                            if (baseProduct.productID.TVal.Equals(failoverProduct.productID.TVal))
                            {
                               ...
                               //find is not safe, not guaranteed that all nodes have been loaded.
                                splRec = client.GetProductLicense(currProductLicense);

                                if (splNumbers.Contains(currProductLicense) && splRec.plState != (byte)ProductLicenseState.Deactivated)
                                    bFound = true;
                                break;
                            }
                        }
                        //just need to delete plentity
                        //resync will reload license node
                        if (!bFound)
                        {  //remove order from database
                            ProductLicenseTable pltDelete = client.GetProductLicense(fplNumber);
                            if (pltDelete != null)
                            //    client.DeleteProductLicense(pltDelete);
                                //DeleteOrderEntity(pltDelete);
                ...
                foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs product in licPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                {
                    ...
                    {
                        splRec = client.GetProductLicense(splNumber);
                        fplRec = client.GetProductLicense(string.Format("{0}-{1}", licData.Name, splRec.plIndex));
                        if (splRec != null) standardID = splRec.ID;
                        if (fplRec != null) failoverID = fplRec.ID;
                        //if (splRec.plState != (byte)ProductLicenseState.Deactivated)
                        //{
                            if (fplRec != null)
                            {   //stored order exists, update
                                fplRec.plState = splRec.plState;
                                fplRec.ProductVersion = splRec.ProductVersion;
                                fplRec.ExpirationDate = splRec.ExpirationDate;
                                fplRec.Description = splRec.Description;
                                //client.UpdateSubLicenseTransactionsByOrder(splRec.ID, fplRec.ID);
                                foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs module in product.moduleList.TVal)
                                {
                                    if (module.contractNumber.TVal.Equals(splNumber))
                                        module.contractNumber.TVal = fplRec.plID;
                                }
                                client.UpdateProductLicense(fplRec);
                            }
                            else
                            {   //storedRec order doesn't exist, create                            
                                splRec.LicenseID = failOverLicenseRec.ID;
                                splRec.plID = string.Format("{0}-{1}", licData.Name, splRec.plIndex);
                                if (splRec.ParentProductLicenseID != null)
                                    splRec.ParentProductLicenseID = splRec.ParentProductLicenseID.Replace(licenseBase, "F");
                                foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs module in product.moduleList.TVal)
                                {
                                    if (module.contractNumber.TVal.Equals(splNumber))
                                        module.contractNumber.TVal = splRec.plID;
                                }
                                client.CreateProductLicense(splRec);
                                fplRec = client.GetProductLicense(string.Format("{0}-{1}", licData.Name, splRec.plIndex));
                                if (fplRec != null) failoverID = fplRec.ID;
                                //client.UpdateSubLicenseTransactionsByOrder(splRec.ID, fplRec.ID);
                            }
                        ...
                        //{                           
                        //    //remove all deactivated modules
                        //    Lic_PackageAttribs.Lic_ProductInfoAttribs.Lic_ModuleInfoAttribsList moduleList = Lic_LicenseInfoAttribsHelper.GetModuleList_ByContractNumber(product, splNumber);
                        //    foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs mod in moduleList.TVal)
                        //    {
                        //        product.moduleList.TVal.Remove(mod);
                        //    }
                        ...
                        //        removeProducts.Add(product);
                        //}
                        client.UpdateSubLicenseTransactionsByOrder(standardID, failoverID);
                    }
                }
                //foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs removeProduct in removeProducts)
                //{
                //    licPackage.licLicenseInfoAttribs.TVal.productList.TVal.Remove(removeProduct);
                //}
        ...
        private bool DeleteProductLicenseNode(TreeNode plNode, bool bVerify)
        {
            ...
            if (plNode != null)
            {
                ...
                if (bDeleteProductLicense)
                {
                    ...
                    Service<ICreator>.Use((client) => { pltDelete = client.GetProductLicense((plNode.Tag as ProductLicenseProperty).ID); });
                    Lic_PackageAttribs.Lic_ProductInfoAttribs storedProduct = DeleteOrderEntity(pltDelete);
        ...
        private Lic_PackageAttribs.Lic_ProductInfoAttribs DeleteOrderEntity(ProductLicenseTable plt)
        {
            ...
            Service<ICreator>.Use((client) =>
            {
                ...
                {                    
                    if (product.productID.TVal.Equals((uint)plt.ProductID))
                    {
                        ...
                        //decrement appinstance for product when removing an order
                        if (s_CommLink.GetProductSpec((uint)plt.ProductID).productLicType.TVal != Lic_PackageAttribs.Lic_ProductSoftwareSpecAttribs.TProductLicenseType.pltClient)
                            if (plt.plState != (byte)ProductLicenseState.AddOn && plt.plState != (byte)ProductLicenseState.Deactivated)
                                product.productAppInstance.TVal -= 1;
                        Lic_PackageAttribs.Lic_ProductInfoAttribs.Lic_ModuleInfoAttribsList moduleList = Lic_LicenseInfoAttribsHelper.GetModuleList_ByContractNumber(product, plt.plID);
                        foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs mod in moduleList.TVal)
                        {
                            product.moduleList.TVal.Remove(mod);
                        }
                ...
                if (storedProduct.moduleList.TVal.Count == 0)
                    licPackage.licLicenseInfoAttribs.TVal.productList.TVal.Remove(storedProduct);
                storedLicense.LicenseInfo = licPackage.Stream;
                client.UpdateLicense(storedLicense);
            });
            //CreateTransaction(TransactionType.ProductLicense, 
            //                  storedLicense.LicenseName,
            //                  string.Format("Removed {0} Product License - {1}", s_CommLink.GetProductName((uint)plt.ProductID), Enum.GetName(typeof(ProductLicenseState),plt.plState)),
            //                  plt.plID);
            return storedProduct;
        }

        //transaction type
        //means updating licenseinfo and removing product nod
           ...
           // retrieve tokens from database not license info object
            ...
            if (tokenList != null)
            {
               // dlvAddToolStripButton.Enabled = (licenseData.LicInfo.licVerificationAttribs.TVal.validationTokenList.TVal.Count > 2) ? false : true;
                ...
                dlvRemoveToolStripButton.Enabled = tokenList.Count > 0;
        ...
        public void PopulateDetailListView(ProductProperty productData)
        {
            ...
            Service<ICreator>.Use((client) =>
            {
                plDeactive = client.GetDeactivatedProductLicenses(lsp.Name, (int)productData.ID);
            });
            foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs module in productData.ModuleList.TVal)
            {
                if (!plDeactive.Contains(module.contractNumber))
                {
                    if (!(module.moduleAppInstance.TVal.Equals(0) && module.moduleValue.TVal.Equals(0)))
                    {
                        ListViewItem lvItem = CreateModuleListViewItem(module, productData.ID, true);
        ...
        private void SetLicenseServerState(TreeNode currentNode, bool bAllLicenseServers)
        {
            ...
            Service<ICreator>.Use((client) =>
            {
                updatedLicenses = client.GetUpdatedLicensesByCustomer(_selectedLicenseCustomer.Name);
        ...
        private void LoadProductNode(TreeNode node)
        {
            ...
            if (node.Parent != null)
            {
                ...
                Service<ICreator>.Use((client) =>
                {
                    plRecords = client.GetProductLicensesByProduct(node.Parent.Name, s_CommLink.GetProductID((node.Tag as ProductProperty).Name));
                });
                TreeNode plParent;
                ...
                foreach (ProductLicenseTable plt in plRecords)
                {
                    ...
                    selectedProduct = new ProductProperty((node.Tag as ProductProperty).Product);
                    if (plt.plState == (byte)ProductLicenseState.Deactivated)
        ...
        private void LoadLicenseNode(TreeNode licenseNode)
        {
            ...
            if (licenseNode.Nodes.Count != licPackage.licLicenseInfoAttribs.TVal.productList.TVal.Count)
            {
                ProductProperty productObj;
                List<TreeNode> removeNodes = new List<TreeNode>();
                foreach (TreeNode productNode in licenseNode.Nodes)
                {
                    productObj = productNode.Tag as ProductProperty;
                    if(productObj != null)
                    {
                        bool bFound = false;
                        foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs prod in licPackage.licLicenseInfoAttribs.TVal.productList.TVal)
                        {
                            if (prod.productID.TVal == productObj.ID)
                            {
                                bFound = true;
                                break;
                            }
        ...
        private IList<LicenseTable> GetDBLicenses(BackgroundWorker worker, string searchString, bool loadOption)
        {
            ...
            Service<ICreator>.Use((client) =>
            {
                try
                {
                    ...
                    licenses = client.GetLicensesByCustomer(_selectedLicenseCustomer.Name, searchString, loadOption);
                }
                catch (Exception e) { }
            });
            return licenses;
        ...
        private void RemoveDeactivatedProductLicenses(ref Lic_PackageAttribs licInfo)
        {
            List<string> productLicenses = null;
            List<Lic_PackageAttribs.Lic_ModuleInfoAttribs> removeModules = new List<Lic_PackageAttribs.Lic_ModuleInfoAttribs>();
            List<Lic_PackageAttribs.Lic_ProductInfoAttribs> removeProducts = new List<Lic_PackageAttribs.Lic_ProductInfoAttribs>();
            //get all product license ids 
            //Lic_PackageAttribs licInfo = new Lic_PackageAttribs();
            //licInfo.Stream = licRec.LicenseInfo;
            foreach (Lic_PackageAttribs.Lic_ProductInfoAttribs product in licInfo.licLicenseInfoAttribs.TVal.productList.TVal)
            {
                ...
                Service<ICreator>.Use((client) =>
                {
                    productLicenses = client.GetDeactivatedProductLicenses(label, (int)product.productID.TVal);
                });
                foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs mod in product.moduleList.TVal)
                {
                    if(productLicenses.Contains(mod.contractNumber.TVal))
                        removeModules.Add(mod);
                }
                foreach (Lic_PackageAttribs.Lic_ModuleInfoAttribs removeMod in removeModules)
                {
                    product.moduleList.TVal.Remove(removeMod);
                }                     
                removeModules.Clear();
                if (product.moduleList.TVal.Count == 0)
                    removeProducts.Add(product);
            ...
            //licRec.LicenseInfo = licInfo.Stream;
        }
        ...
        private void GenerateDBPacket(string packetName, string licenseName, DateTime expDate, string description, string outputPath)
        {
            Service<ICreator>.Use((client) => 
            {
                ...
                RemoveDeactivatedProductLicenses(ref licPackage);
                licRec.LicenseInfo = licPackage.Stream;
        ...
        private void editDestinationNamesToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void LoadSelectedLicenseServer(string selectedLicense)
        {
            ...
            Service<ICreator>.Use((client) =>
            {
                ...
                _selectedLicenseCustomer.Name = client.GetCustomer(custID.ToString(), false).SCRname;
        ...
        private void LoadLicenseServerReport(List<Condition> conditionList)
        {
            ...
            if (licenses != null)
            {
                ...
                for (int index = 0; index < licenses.Count; index++)
                {
                    ...
                    newItem.SubItems.Add(((LicenseServerType)(licenses[index].LicenseType)).ToString());
                    lvItems[index] = newItem;
                }
                ReportListView.Items.AddRange(lvItems);
            }
        ...
        private void ReportListView_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {                
                ...
                if (ReportListView.Columns.Count == 3)
                {
                    if(item.SubItems[1].Text != "")
                        LoadSelectedLicenseServer(item.SubItems[1].Text);
                }
