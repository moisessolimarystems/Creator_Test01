//
// Lic_ServerDataFileInfoAttribs_Helper.h
// Wrapper class for the  Lic_ServerDataAttribs::Lic_ServerDataFileInfoAttribs class.
//
#pragma once
#include "Lic_ServerDataAttribs.h"

/*
 * class Lic_ServerDataFileInfoAttribs_Helper
 *
 *
 */
typedef Lic_ServerDataAttribs::Lic_ServerDataFileInfoAttribs TLSDFI_Attribs;
class Lic_ServerDataFileInfoAttribs_Helper : public TLSDFI_Attribs
{
public:
	Lic_ServerDataFileInfoAttribs_Helper() :
		TLSDFI_Attribs()
	{
	}
	Lic_ServerDataFileInfoAttribs_Helper(const TLSDFI_Attribs& cpy) :
		TLSDFI_Attribs(cpy)
	{
		NormalizeFileLicenseCode();
	}

	/* SetLicFileLicenseCode() - over-rides attribs default method and normalizes GUID
	 ...
	 * NormalizeFileLicenseCode(). Converted this method to use that.
	 --------------------------------------------------------------------------------*/
	virtual void SetLicFileLicenseCode(
		const CSpdCString& val)
	{
		// If no curly bracket for this GUID, then just thunk attribs method.
		if (IsNormalizedCode(val)) { TLSDFI_Attribs::SetLicFileLicenseCode(val); return; } // CR.34180; modified. CR.34040.V01; modified.

		// remove curly brackets at begining and end, before calling attribs method.
		TLSDFI_Attribs::SetLicFileLicenseCode(GetNormalizedCode(val));						// CR.34180; modified.
	}
		...
		const CSpdCString& wsLicenseCode) const
	{
		...
		return wsNormalized.compare(GetLicFileLicenseCode(), false) == 0;
	}

private:
	/* NormalizeFileLicenseCode() - reads FileLicenseCode and rewrites it to ensure
	 * it gets normalized (curly brackets removed)
	 *
	 * SLB 18.jun.2025 CR.34040; Created.
	 --------------------------------------------------------------------------------*/
	void NormalizeFileLicenseCode()
	{
		SetLicFileLicenseCode(GetLicFileLicenseCode());
	}
